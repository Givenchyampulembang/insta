...
export default function Notification() {
  ...
  const { data, isLoading } = UseQuery(
    `${process.env.NEXT_PUBLIC_ENDPOINT}/notifications`
  );
  ...
  useEffect(() => {
    ...
    data?.map((item) => {
      if (
        tmp[moment(item.created_at, "YYYY-M-D").format("DD MMMM YYYY")] ===
        undefined
      ) {
        tmp[moment(item.created_at, "YYYY-M-D").format("DD MMMM YYYY")] = [
          item,
        ...
        tmp[moment(item.created_at, "YYYY-M-D").format("DD MMMM YYYY")].push(
          item
        );
      }
    });
                          ...
                          <CardBody>
                            <Flex gap={2}>
                              ...
                              >
                                ...
                                <Box className="mt-2">{item?.user?.name}</Box>
                              </Flex>
                              <Box className="mt-4">
