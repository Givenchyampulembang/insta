    ...
    setContentModal(
      <div>
        <form>
          ...
          <ModalBody>
            <FormControl>
              <Textarea ref={inputDescription}>{desc}</Textarea>
            </FormControl>
          </ModalBody>
          <ModalFooter>
            {isLoading ? (
  ...
  const deletePost = (id) => {
    setContentModal(
      <div>
        ...
        <ModalCloseButton />
        <ModalBody>Delete this post ?</ModalBody>
        <ModalFooter>
          {isLoading ? (
            <Button className="w-full" disabled>
              <DeleteIcon className="mr-1" /> Deleting it...
            </Button>
          ) : (
            <Button
              onClick={async () => {
    ...
    <Card className="mb-3">
      ...
      <CardFooter>
        <Flex className="w-full justify-center" gap={2}>
          <Box
            onClick={() => {
              likes(id, is_like_post);
            }}
            className="w-full flex hover:text-blue-700 hover:cursor-pointer"
          >
            {is_like_post ? (
            ...
            <span
              className={`ml-1 ${
                is_like_post ? "text-blue-700" : "text-black"
              }`}
            >
              {likes_count > 0 ? `${likes_count}` : ""} Likes
            </span>
          </Box>
          <Spacer />
          <Box
            ...
            className="hover:cursor-pointer hover:text-blue-700 w-full flex"
          >
