...
export default function Replies({ id, onClose }) {
  const { isLoading, data } = UseQuery(
    `${process.env.NEXT_PUBLIC_ENDPOINT}/replies/post/${id}`
  );
                  ...
                  <VStack className="float-left">
                    ...
                    <Box className="w-full">
                      {item?.is_own_reply ? (
                        <div
                          onClick={() => {
                            deleteReplies(item.id);
                          }}
                          className="text-xs text-left mt-5 text-gray-300 hover:cursor-pointer hover:text-gray-700"
                        >
